...
class DAO:
    ...
    def connect(self):
        connect_string = "dbname=" + self.dbname + " user=" + self.username + " host=127.0.0.1 port=5432 password='" + self.password + "'"
        try:
...
c = DAO('postgres', 'test_wine', 'password')
c.connect()
c.insert_json('fulldata.json')
# c.select_all('SELECT * FROM wine6')
"""c.create_table('review',IDnum='serial NOT NULL PRIMARY KEY ',points='integer NOT NULL', title='varchar(255)', description="varchar(1000)", taster_name="varchar(50)",
               taster_twitter_handle="varchar(255)", price="integer", designation="varchar(255)", variety="varchar(255)", region_1="varchar(255)",
               region_2="varchar(255)",
               province="varchar(255)", country="varchar(255)", winery="varchar(255) NOT NULL")"""
c.connection.close()